...
from resnet_cifar import resnet32, resnet20, resnet9
...
def convnet_pre_bn(dataset_name: str, widths: List[int], activation: str, pooling: str, bias: bool) -> nn.Module:
    modules = []
    size = image_size(dataset_name)
    for l in range(len(widths)):
        prev_width = widths[l - 1] if l > 0 else num_input_channels(dataset_name)
...
def convnet_ln(dataset_name: str, widths: List[int], activation: str, pooling: str, bias: bool) -> nn.Module:
    modules = []
    size = image_size(dataset_name)
    for l in range(len(widths)):
        prev_width = widths[l - 1] if l > 0 else num_input_channels(dataset_name)
...
def convnet_pre_ln(dataset_name: str, widths: List[int], activation: str, pooling: str, bias: bool) -> nn.Module:
    modules = []
    size = image_size(dataset_name)
    for l in range(len(widths)):
        prev_width = widths[l - 1] if l > 0 else num_input_channels(dataset_name)
...
def load_architecture(arch_id: str, dataset_name: str) -> nn.Module:
    ...
    elif arch_id == 'conv_bn_32':
        return convnet_bn(dataset_name, [32, 32], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_pre_bn_32':
        return convnet_pre_bn(dataset_name, [32, 32], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_ln_32':
        return convnet_ln(dataset_name, [32, 32], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_pre_ln_32':
        return convnet_pre_ln(dataset_name, [32, 32], activation='relu', pooling='max', bias=True)

    elif arch_id == 'conv_64':
        return convnet(dataset_name, [64, 64], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_bn_64':
        return convnet_bn(dataset_name, [64, 64], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_pre_bn_64':
        return convnet_pre_bn(dataset_name, [64, 64], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_ln_64':
        return convnet_ln(dataset_name, [64, 64], activation='relu', pooling='max', bias=True)
    elif arch_id == 'conv_pre_ln_64':
        return convnet_pre_ln(dataset_name, [64, 64], activation='relu', pooling='max', bias=True)
